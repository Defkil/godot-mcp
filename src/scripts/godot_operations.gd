...
# Raw resource paths are intentionally rejected to prevent agent-controlled
# scripts from being loaded and instantiated through node type parameters.
func get_script_by_name(name_of_class):
    if debug_mode:
        print("Attempting to get script for registered class: " + name_of_class)

    # Search only the project's global class registry.
