...
describe('MCP schema and dispatch parity', () => {
  it('advertises all 141 legacy contracts plus the 17 migrated/registered tools for exactly 158 unique names', async () => {
    ...
    // 141 legacy contracts + the migrated/registered tools: modify_project_settings,
    ...
    // rename_file, game_connect_signal, game_disconnect_signal.
    ...
    expect((server as any).toolRegistry.capabilityFor('write_file')).toBe('edit');
    ...
    expect((server as any).toolRegistry.capabilityFor('game_disconnect_signal')).toBe('edit');
  });
