...
| [tugcantopaloglu#12](https://github.com/tugcantopaloglu/godot-mcp/issues/12) schemas advertise phantom commands/actions | closed | partial | A typed registry now derives schema and dispatch for migrated tools. Real MCP list coverage confirms 158 unique advertised names, and real MCP call coverage exercises migrated handlers, including a project-settings mutation with independent file read-back. The eight migrated tools are `modify_project_settings`, `list_project_files`, `launch_editor`, `read_scene`, `modify_scene_node`, `remove_scene_node`, `classdb_inspect`, and `get_project_info`; each is registered in `toolRegistry` and removed from the legacy switch and flat-list block, leaving 150 legacy `case` statements in `src/server.ts:3540` and 150 flat-list entries. Complete migration must remove the remaining legacy source-text switch assertions. |
