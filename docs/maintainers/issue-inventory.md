...
| [tugcantopaloglu#12](https://github.com/tugcantopaloglu/godot-mcp/issues/12) schemas advertise phantom commands/actions | closed | partial | A typed registry now derives schema and dispatch for the first migrated tool. Real MCP list coverage confirms 157 unique advertised names, and real MCP call coverage exercises that migrated handler. Complete migration must remove the remaining legacy source-text switch assertions. |
...
5. Most handler tests inspect source strings and regular expressions; they do not execute handlers or Godot. The first registry migration now executes `list_project_files` through the real MCP list/call boundary, but the remaining legacy tools still require behavioral parity coverage.
