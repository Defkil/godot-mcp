...
4. `godot_operations.gd` is a 1,887-line dispatcher. Legacy handlers still infer success from stderr text, but the takeover now has a bounded streaming TypeScript operation-runner that requires process success plus a typed result marker, enforces time/output/argument limits, observes startup errors and terminates timed-out process trees. Handler migration and operation-specific postcondition checks remain open.
