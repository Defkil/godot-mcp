...
| [tugcantopaloglu#12](https://github.com/tugcantopaloglu/godot-mcp/issues/12) schemas advertise phantom commands/actions | closed | partial | A typed registry now derives schema and dispatch for the first two migrated tools. Real MCP list coverage confirms 157 unique advertised names, and real MCP call coverage exercises both migrated handlers, including a project-settings mutation with independent file read-back. Complete migration must remove the remaining legacy source-text switch assertions. |
